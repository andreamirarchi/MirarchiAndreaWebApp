...
public class PiattoDaoJDBC implements PiattoDao {
    ...
    public List<Piatto> findAll() {
        ...
        try {
            ...
            while (rs.next()) {
                // Usa PiattoProxy invece di Piatto
                Piatto piatto = new PiattoProxy(rs.getString("nome"), rs.getString("ingredienti"));

                // Non è necessario chiamare getIngredienti() qui,
                // dato che PiattoProxy gestisce il caricamento degli ingredienti in modo lazy
...
ON CONFLICT (id) DO UPDATE SET txt = EXCLUDED.txt;
    ...
    public static void main(String[] args) {
        ...
        Piatto piattoDaEliminare = piatti.get(0);
        piattoDao.delete(piattoDaEliminare);
        System.out.println("Piatto " + piattoDaEliminare.getNome() + " eliminato.");

        piatti = piattoDao.findAll();
        System.out.println("Lista dei piatti dopo la cancellazione:");
        for (Piatto piatto : piatti) {
            System.out.println("Piatto: " + piatto.getNome());
            System.out.println("Ingredienti: " + piatto.getIngredienti());
        }
